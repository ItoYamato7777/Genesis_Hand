...
class ShadowHandEnv:
    ...
    def step(self, actions):
        ...
        self.process_actions(clip_actions=self.actions)
        self.hand.control_dofs_position(self._processed_actions, self.motor_dofs)
        #print("processed_actions", self._processed_actions)

        self.scene.step()
        # update buffers

        self.episode_length_buf += 1
        self.hand_pos[:] = self.hand.get_pos()
        self.hand_quat[:] = self.hand.get_quat()
        self.dof_pos[:] = self.hand.get_dofs_position(self.all_dofs)
        self.dof_vel[:] = self.hand.get_dofs_velocity(self.all_dofs)
        # self.fingertip_pos = self.hand_pos[:, ] - env.scene.env_origins.unsqueeze(1).repeat(1, 4, 1).reshape(self.num_envs, -1)

        self.target_pos[:] = self.target.get_pos()
        self.object_pos[:] = self.object.get_pos()
        self.target_quat[:] = self.target.get_quat()
        self.object_quat[:] = self.object.get_quat()
        self.object_lin_vel[:] = self.object.get_vel()
        self.object_ang_vel[:] = self.object.get_ang()
        ...
        reset_target_pose_envs_ids = (self.rot_dist <= self.success_tolerance).nonzero(as_tuple=False).flatten()
        # reset goals if the goal has been reached
        if len(reset_target_pose_envs_ids) > 0:
            self.reset_target_pose(reset_target_pose_envs_ids)

        # reset pos if the object has fallen
        out_of_reach_envs_ids = (self.goal_dist > self.fall_dist).nonzero(as_tuple=False).flatten()
    ...
    def reset_idx(self, envs_idx):
        ...
        self.object_init_quat = torch.tensor(random_object_reset_quat, device=self.device)
        ...
        self.target_init_quat = torch.tensor(random_target_reset_quat, device=self.device)
    ...
    def _reward_rotation(self):
        return 1.0 / (torch.abs(self.rot_dist) + self.rot_eps + 1e-6)

    # 4.成功ボーナス
    def _reward_reach_goal_bonus(self):
        return torch.where(torch.abs(self.rot_dist) <= self.success_tolerance, self.reach_goal_bonus, torch.zeros_like(self.rot_dist))

    # 5.落下ペナルティ
    def _reward_fall_penalty(self):
        return torch.where(self.goal_dist > self.fall_dist, self.fall_penalty, torch.zeros_like(self.goal_dist))
    ...
    def process_actions(self, clip_actions: torch.Tensor):
        ...
        self._processed_actions[:] = torch.clamp(
            targets,
            self.all_dof_pos_limits[:,  0].repeat(self.num_envs, 1),
            self.all_dof_pos_limits[:,  1].repeat(self.num_envs, 1),
        )
...
def generate_random_quaternions(num_quats: int, device="cpu") -> torch.Tensor:
    ...
    ランダムなクォータニオンを生成する。
    Args:
        num_quats: 生成するクォータニオンの数。
        device: 生成するテンソルのデバイス ("cpu" または "cuda")。
    Returns:
        生成されたクォータニオン。形状は (num_quats, 4)。
    """
...
def quaternion_difference(object_rot, target_rot):
    return quat_mul(object_rot, quat_conjugate(target_rot))
...
def rotation_distance(object_rot, target_rot):
    ...
    return 2.0 * torch.asin(torch.clamp(norm_result, max=1.0))  # changed quat convention